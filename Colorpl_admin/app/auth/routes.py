...
async def signup(signup_data: AdministratorCreate, db: Session = Depends(get_db)):
    existing_admin = db.query(Administrator).filter(Administrator.email == signup_data.email).first()
    if existing_admin:
        raise HTTPException(
            status_code=400,
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    admin = db.query(Administrator).filter(Administrator.email == form_data.username).first()
...
@router.get("/admin/administrators", response_model=List[AdministratorSchema], dependencies=[Depends(super_admin_required)])
async def list_administrators(db: Session = Depends(get_db)):
    admins = db.query(Administrator).all()
    return [AdministratorSchema.from_orm(admin) for admin in admins]
