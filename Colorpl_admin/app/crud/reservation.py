...
from models import Reservation, Member, ReservationDetail, Seat, ShowSchedule, ShowDetail, Hall
...
def update_reservation(db: Session, reservation_id: int, reservation_update: ReservationUpdate):
    ...
    # reservation.nickname = member.nickname if member else None
    # reservation.profile = member.profile if member else None

    # seat = db.query(Seat).filter(Seat.seat_id == reservation.seat_id).first()
    # reservation.seat_col = seat.col if seat else None
    # reservation.seat_row = seat.row if seat else None

    # show_detail = db.query(ShowDetail).filter(ShowDetail.show_detail_id == reservation.show_detail_id).first()
    # reservation.show_detail_name = show_detail.name if show_detail else None

    # hall = db.query(Hall).filter(Hall.hall_id == reservation.hall_id).first()
    # reservation.hall_name = hall.name if hall else None
