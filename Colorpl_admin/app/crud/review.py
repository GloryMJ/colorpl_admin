...
def get_reviews_activity(db: Session, skip: int = 0, limit: int = 10):
    ...
    for review in reviews:
        ...
        is_spoiler = bool(int.from_bytes(review.is_spoiler, byteorder='big'))
        review_list.append({
...
def get_review_by_id(db: Session, review_id: int):
    try:
        ...
        print(f"Error occurred in get_review_by_id: {e}")
        raise e
...
def update_review(db: Session, review_id: int, review_update: ReviewUpdateDTO):
    ...
    try:
        ...
        for key, value in update_data.items():
            if key == 'is_spoiler':
                setattr(review, key, bytes([int(value)]))  # 0 또는 1로 저장
            else:
